...
<c:choose>
    <c:when test="${(sessionScope.user eq null) or (sessionScope.user.role eq 'GUEST')}">
        <form method="POST" action="/WebPark/Controller?command=logIn" accept-charset="UTF-8">
    ...
    <c:otherwise>
        <form method="POST" action="/WebPark/Controller?command=logOut" accept-charset="UTF-8">
